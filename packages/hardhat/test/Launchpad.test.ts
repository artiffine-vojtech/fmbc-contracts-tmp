...
import { ControllerFactory__factory, ERC20, IVault, IdentityVerifier__factory, Launchpad, MEMEVesting, NFTChecker__factory, TokenEmissionsController, TokenIncentivesController, TokenIncentivesController__factory, Launchpad__factory, UniswapV2DexProvider__factory, BalancerDexProvider__factory } from '../../hardhat-types/src'
...
describe('Launchpad', function () {
  ...
  async function deployContractFixture() {
    ...
    await router.connect(owner).addLiquidity(options.tokenA, options.tokenB, options.amountTokenA, options.amountTokenB, 1, 1, owner.address, 9678825033);

    const fomoUsdcLpAddress = await factory.getPair(fomo.address, usdc.address)
    console.log(fomoUsdcLpAddress)
    const fomoUsdcLp = await ethers.getContractAt("IERC20", fomoUsdcLpAddress, owner) as ERC20;
    ...
    const uniswapDexProvider = await uniswapDexProviderFactory.deploy(
      router.address, factory.address
    )
    ...
    const usdcTotal = await uniswapDexProvider.getPoolBalance(fomoUsdcLp.address, usdc.address);
  ...
  describe('Pledge', async () => {
    ...
    describe('IN KOL Phase', async () => {
      ...
      it('Should emit Pledged event correctly', async () => {
        ...
        await fomoUsdcLp.connect(user).approve(launchpad.address, constants.MaxUint256)
        const action = launchpad.connect(user).pledge(0, LP_500, false, SIGNED_DATA[1])
        await expect(action).to.emit(launchpad, 'Pledged').withArgs(0, user.address, LP_500, parseUnits("500", 6))
      })
  ...
  describe.only('Launch', async () => {
    ...
    it('Should send proper amount of tokens to Balancer Vault', async () => {
      const { launchpad, user, fomo, usdc, balancerVaultAddress, owner } = await loadFixture(deployContractFixtureWithHardCapReached)
  ...
  describe('ClaimTokens', async () => {
    ...
    it('Should transfer correct token amount', async () => {
      ...
      for (let i = 5; i < 249; i++) {
        ...
        expect(await meme.balanceOf(pledgers[i].address)).to.be.eq(userMemeTokensAmount)
      }
      await launchpad.connect(owner).claimTokens(0)
      expect(await meme.balanceOf(owner.address)).to.be.eq(ownerBalanceBefore.add(ownerPledge.lp.mul(memeTotalSupply).mul(47).div(launch.raisedLP).div(100)))
    ...
    it('Should set user pledge claimed amunt properly', async () => {
      ...
      const launch = await getLaunchConfig(launchpad, 0)
      const userPledge = await launchpad.launchToUserPledge(0, user.address)
      expect(userPledge.claimed).to.be.eq(userPledge.lp.mul(launch.totalSupply).mul(1e12).mul(1e6).mul(47).div(launch.raisedLP).div(100));
    })
  })
