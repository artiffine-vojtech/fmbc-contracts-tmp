...
describe('Launchpad-Uniswap', function () {
  ...
  describe('Launch', async () => {
    ...
    it('Should create meme-fomo LP with correct amounts', async () => {
      const { launchpad, user, fomo } = await loadFixture(deployContractFixtureWithHardCapReached)
      await launchpad.connect(user).launch(0)
      const alloc = await launchpad.tokenAddresses(0)
      ...
      expect(await meme.balanceOf(memeFomoLP.address)).to.be.eq((await meme.totalSupply()).mul(3525).div(20000))
    })

    it('Should create meme-usdc LP with correct amounts', async () => {
      const { launchpad, user, usdc } = await loadFixture(deployContractFixtureWithHardCapReached)
      await launchpad.connect(user).launch(0)
      const alloc = await launchpad.tokenAddresses(0)
      ...
      expect(await meme.balanceOf(memeUsdcLP.address)).to.be.eq((await meme.totalSupply()).mul(3525).div(20000))
    })
    ...
    it('Should send both LPs to DEAD address', async () => {
      ...
      const memeUsdcLP = await env.ethers.getContractAt("ERC20", alloc.usdcLP) as ERC20;
      const memeFomoLP = await env.ethers.getContractAt("ERC20", alloc.fomoLP) as ERC20;
      expect(await memeFomoLP.balanceOf(DEAD_ADDRESS)).to.be.eq((await memeFomoLP.totalSupply()).sub(1000))
      expect(await memeUsdcLP.balanceOf(DEAD_ADDRESS)).to.be.eq((await memeUsdcLP.totalSupply()).sub(1000))
    ...
    describe('Meme allocations', async () => {
      it('With zero team allocation', async () => {
        ...
        const memeUsdcLP = await env.ethers.getContractAt("ERC20", alloc.usdcLP) as ERC20;
        const memeFomoLP = await env.ethers.getContractAt("ERC20", alloc.fomoLP) as ERC20;
        expect(await meme.balanceOf(memeUsdcLP.address)).to.be.eq((await meme.totalSupply()).mul(config.allocations[3]).div(20000))
        expect(await meme.balanceOf(memeFomoLP.address)).to.be.eq((await meme.totalSupply()).mul(config.allocations[3]).div(20000))
      })
