...
contract TokenEmissionsController is ITokenIncentivesController, Adminable {
    ...
    uint public immutable LOCK_TIME;
    ...
    constructor(IERC20 _stakingToken, INFTWithLevel _boosterNFT, address _rewardToken, uint _lockTime) Adminable() {
    ...
    function startEmissions(EmissionPoint[] memory _emissions) external {
        ...
        _notifyReward(rewardTokens[0], emissions[currentEmissionsIndex].amount);
    }
    ...
    function deposit(uint _amount, address _onBehalfOf) external {
        require(msg.sender == _onBehalfOf, 'Not allowed');
        ...
        userLockTime[_onBehalfOf] = block.timestamp + LOCK_TIME;
    ...
    function setRewardsDuration(uint256 _newRewardsDuration) external onlyOwner {
        _setRewardsDuration(_newRewardsDuration);
    }
